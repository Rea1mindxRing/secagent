...
5. 让另一位朋友 **Review（审查）** 代码
6. 审查通过后，点 **"Merge pull request"** → **"Confirm merge"**
...
| 行为 | 后果 |
|------|------|
| 直接在 `main` 上改代码 | 代码混乱，无法审查 |
| `git push origin main` 直接推 main | 绕过审查，严禁 |
| `git add .` 不看改了啥就提交 | 容易把不该提交的文件带进去 |
| 提交信息写 "aaa"、"fix"、"111" | 别人看不懂，以后自己也看不懂 |
| 冲突了不会解决就硬推（`--force`） | 会覆盖别人的代码，**非常危险** |
